...
app.post('/upload', fileUpload.single('file'), async (req, res) => {
  try {
    ...
    if (result.message.conversationType === 'room') {
      ...
      // 私聊发送给双方
      socket.to(result.message.conversationId).emit('newMessage', result.message);
...
// 提供test-chat.html文件访问
app.get('/test-chat.html', (req, res) => {
  const testChatPath = path.join(__dirname, '../test-chat.html');
  if (fsSync.existsSync(testChatPath)) {
    res.sendFile(testChatPath);
  } else {
  ...
  socket.on('login', async (username) => {
    try {
      ...
      if (existingUser) {
        ...
        await Message.destroy({ where: { sender: existingUser.id } });
        await File.destroy({ where: { owner: existingUser.id } });
        await Friend.destroy({ where: { userId: existingUser.id } });
        await Friend.destroy({ where: { friendId: existingUser.id } });
        // 删除离线用户记录
  ...
  socket.on('sendMessage', async (data) => {
    try {
      ...
      if (message.conversationType === 'room') {
        // 确保用户确实在房间中
        const isInRoom = await RoomMember.findOne({
          where: { roomId: message.conversationId, userId: socket.id }
        });
        
        if (isInRoom) {
          io.to(message.conversationId).emit('newMessage', message);
          console.log(`[消息] 发送房间消息到 ${message.conversationId}, 发送者: ${user.username}`);
        } else {
          console.warn(`[消息警告] 用户 ${user.username} 不在房间 ${message.conversationId} 中，消息未发送`);
          socket.emit('messageError', '你不在该房间中，无法发送消息');
        }
      } else {
        // 私聊发送给双方
        socket.to(message.conversationId).emit('newMessage', message);
        socket.emit('newMessage', message);
        console.log(`[消息] 发送私聊消息从 ${user.username} 到 ${message.conversationId}`);
      }
      
    } catch (error) {
      console.error(`[消息错误] ${error.message}`);
      socket.emit('messageError', '发送消息失败，请重试');
    }
  });
...
app.get('/test-join-room', async (req, res) => {
  try {
    console.log('[测试端点] 收到房间加入请求');
    const roomId = req.query.roomId || 'test-room-1';
    console.log(`[测试端点] 房间ID: ${roomId}`);

    // 确保roomId是字符串
    const stringRoomId = String(roomId);
    console.log(`[测试端点] 转换后的房间ID: ${stringRoomId}`);

    // 检查房间是否存在，如果不存在则创建
    let room = await Room.findByPk(stringRoomId);
    if (!room) {
      console.log(`[测试端点] 房间不存在，创建新房间: ${stringRoomId}`);
      try {
        room = await Room.create({
          id: stringRoomId,
          name: '测试房间-' + stringRoomId,
          creator: 'system'
        });
        console.log(`[测试端点] 创建了新房间: ${room.id}`);
      } catch (createError) {
        console.error(`[测试端点] 创建房间失败: ${createError.message}`);
        console.error(createError.stack);
        return res.status(500).json({ success: false, error: '创建房间失败' });
      }
    } else {
      console.log(`[测试端点] 房间已存在: ${room.id}`);
    }

    // 创建测试用户
    const username = 'test-user-' + Math.random().toString(36).substr(2, 5);
    console.log(`[测试端点] 测试用户名: ${username}`);
    ...
    try {
      ...
      if (!user) {
        ...
        console.log(`[测试端点] 使用现有测试用户: ${user.id}`);
      }
    } catch (userError) {
      console.error(`[测试端点] 用户操作失败: ${userError.message}`);
      console.error(userError.stack);
      return res.status(500).json({ success: false, error: '用户操作失败' });
    }
    ...
    try {
      const existingMember = await RoomMember.findOne({
        where: { roomId: stringRoomId, userId }
      });

      if (existingMember) {
        console.log(`[测试端点] 用户 ${userId} 已加入房间 ${stringRoomId}`);
        return res.status(400).json({ success: false, error: '用户已加入该房间' });
      }
    } catch (memberCheckError) {
      console.error(`[测试端点] 检查房间成员失败: ${memberCheckError.message}`);
      console.error(memberCheckError.stack);
      return res.status(500).json({ success: false, error: '检查房间成员失败' });
    }
    ...
    try {
      ...
      console.log(`[测试端点] 用户 ${userId} 成功加入房间 ${stringRoomId}`);
    } catch (joinError) {
      console.error(`[测试端点] 加入房间失败: ${joinError.message}`);
      console.error(joinError.stack);
      return res.status(500).json({ success: false, error: '加入房间失败' });
    }

    // 模拟WebSocket事件通知
    try {
      io.to(stringRoomId).emit('userJoined', {
        userId,
        username
      });
      console.log(`[测试端点] 发送用户加入事件到房间 ${stringRoomId}`);
    } catch (socketError) {
      console.error(`[测试端点] 发送WebSocket事件失败: ${socketError.message}`);
    ...
    res.status(500).json({ success: false, error: '测试端点内部错误' });
  }
});
...
server.listen(PORT, () => {
  console.log(`[服务器] 成功启动在 http://localhost:${PORT}`);
  console.log(`[服务器] 环境: ${process.env.NODE_ENV || 'development'}`);
  console.log(`[服务器] PID: ${process.pid}`);
}).on('error', (e) => {
  if (e.code === 'EADDRINUSE') {
    console.error(`[服务器错误] 端口 ${PORT} 已被占用`);
    // 尝试使用其他端口
    const randomPort = 3000 + Math.floor(Math.random() * 1000);
    server.listen(randomPort, () => {
      console.log(`[服务器] 成功启动在 http://localhost:${randomPort}`);
      console.log(`[服务器] 环境: ${process.env.NODE_ENV || 'development'}`);
      console.log(`[服务器] PID: ${process.pid}`);
    });
  } else {
